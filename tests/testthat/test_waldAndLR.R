...
tmpdat <- cbind(10*eurusd[,1], 100*eurusd[,2])
...
mod222s <- GMVAR(tmpdat, p=2, M=2, params=params222s, structural_pars=list(W=W_222))
...
test_that("Wald_test works correctly", {
  ...
  expect_equal(unname(wald1$statistic), 4.147194, tolerance=1e-1)
  expect_equal(wald1$p.value, 0.1257327, tolerance=1e-1)
})
...
params222sc <- c(1.786, 3.001, 1.031, 2.357, 1.25, 0.06, 0.036, 1.335,
                 ...
                 0.14, 0.768, 0.632)
mod222sc <- GMVAR(tmpdat, p=2, M=2, params=params222sc, constraints=C_mat, structural_pars=list(W=W_222))

lr1 <- LR_test(mod222s, mod222sc)

test_that("LR_test works correctly", {
  expect_equal(unname(lr1$parameter), 8)
  expect_equal(unname(lr1$statistic), 18.91746, tolerance=1e-4)
  expect_equal(lr1$p.value, 0.01530736, tolerance=1e-4)
})
