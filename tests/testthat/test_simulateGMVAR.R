...
test_that("simulateGMVAR works correctly", {
  expect_equal(sim_112$sample[1,], c(-6.673635, 113.393216), tolerance=1e-5)
  expect_equal(sim_112$component, 1)
  expect_equal(sim_112$mixing_weights, as.matrix(1))
  expect_equal(sim_222$sample[3,], c(-0.6541336, 119.4602948), tolerance=1e-5)
  expect_equal(sim_222$component, c(2, 2, 2))
  expect_equal(sim_222$mixing_weights[3,], c(0.1486888, 0.8513112), tolerance=1e-5)

  expect_equal(sim_112_2$sample[3, , 3], c(-10.93703, 109.45739), tolerance=1e-5)
  expect_equal(sim_112_2$component[,1], c(1, 1 ,1))
  expect_equal(sim_112_2$mixing_weights[, , 3], c(1, 1, 1))
  expect_equal(sim_222_2$sample[1, , 2], c(4.186998, 116.642899), tolerance=1e-5)
