...
plot.gsmvar <- function(x, ...) {
  ...
  alphas <- pick_alphas(p=p, M=M, d=d, params=gsmvar$params)
  means <- get_regime_means(gsmvar) # Regs in cols, d in rows
  vars <- vapply(1:M, function(m) diag(get_regime_autocovs(gsmvar)[, , 1, m]), numeric(d)) # Regs in cols, d in rows
