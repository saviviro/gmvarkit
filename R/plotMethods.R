...
plot.gmvarpred <- function(x, ..., nt, add_grid=TRUE) {
  ...
  if(gmvarpred$ci_type == "none") {
    all_val <- lapply(1:d, function(j) c(ts_dat[,j], ts_pred[,j]))
  } else {
    all_val <- lapply(1:d, function(j) c(ts_dat[,j], ts_pred[,j], simplify2array(gmvarpred$conf_ints, higher=TRUE)[, j, ]))
  }
  ...
  if(gmvarpred$ci_type == "two-sided") {
    ts_lowers <- lapply(1:(length(q)/2), function(i1) make_ts(gmvarpred$conf_ints[[i1]]))
    ts_uppers <- lapply((length(q)/2 + 1):length(q), function(i1) make_ts(gmvarpred$conf_ints[[i1]]))
    ...
    ts_uppers <- lapply(1:length(q), function(i1) make_ts(gmvarpred$conf_ints[[i1]]))

  } else if(gmvarpred$ci_type == "lower") {
    ts_max <- vapply(1:d, function(j) rep(round(max(all_val[[j]])) + 3, times=nrow(ts_pred)), numeric(nrow(ts_pred)))
    ts_lowers <- lapply(1:length(q), function(i1) make_ts(gmvarpred$conf_ints[[i1]]))
    ts_uppers <- lapply(1:length(q), function(i1) make_ts(ts_max)[-1,])
  }

  for(i1 in 1:d) {
    plot_pred(i1)
    if(gmvarpred$ci_type %in% c("two-sided", "upper", "lower")) {
      for(i2 in 1:length(gmvarpred$ci)) {
