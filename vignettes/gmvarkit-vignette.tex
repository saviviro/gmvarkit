...
\textbf{Another important thing to know about estimation is that the estimation algorithm performs very poorly if some of the AR coefficients very large, signifcantly larger than one. This means that you need scale each component time series so that they vary approximately in the same magnite. For instance, typically in macroeconomic time series, log-differences should be multiplied by hundred. You can try out different scales and estimate a simple linear VAR with your favourite package to see whether the AR coeffients are in a reasonable range.}
...
\end{Sinput}
\end{Schunk}
\includegraphics{gmvarkit-vignette-plot}
%
